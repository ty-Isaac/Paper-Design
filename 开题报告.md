...
```bash
docker swarm init --advertise-addr 192.168.239.129
```
    需要注意IP地址，远程外网应该可以访问。初始化 Swarm 管理节点后会生成一个唯一 token，并且将生成加入集群的命令

2. 创建 worker 节点，并加入 swarm 集群

```bash
docker swarm join     --token SWMTKN-1-24bbzf6i9gac320jbpejrueutr3coa1cs0xdam1f3qbiup67do-bgh6vveuezlos7q7o5zkqd9oh     192.168.1.2:2377
```
3. 移除节点 
